...
public class TablingSolver extends AbstractSolver {
     ...
     */
    private solver.TableEntry[] onTable = null;
    /**
     * One table for every premise. Indices are in format "nnm" with "nn" the
     * index of the cell and "m" the candidate. This table holds all entries for
     * "candidate m deleted from cell nn".
     */
    private solver.TableEntry[] offTable = null;
    /**
     ...
     * checks.
     */
    private List<solver.TableEntry> entryList = new ArrayList<solver.TableEntry>(10);
    /**
     ...
     */
    private List<solver.TableEntry> extendedTable = null;
    /**
    ...
    /**
     * Global chain for {@link #buildChain(solver.TableEntry, int, int[], boolean, SudokuSet)}.
     */
     ...
     */
    private solver.TableEntry[] minEntries = new solver.TableEntry[mins.length];
    /**
     ...
     * main.java.solver is actually used.<br>
     *
    ...
    private void initialize() {
        if (!initialized) {
            onTable = new solver.TableEntry[810];
            offTable = new solver.TableEntry[810];
            for (int i = 0; i < onTable.length; i++) {
                onTable[i] = new solver.TableEntry();
                offTable[i] = new solver.TableEntry();
            }

            extendedTable = new ArrayList<solver.TableEntry>();
    ...
    /**
     * Releases memory, if the main.java.solver has not been used for more than
     * {@link #CLEANUP_INTERVAL} ms.<br>
     *
     * Please note, that this method is called from a seperate thread and must
     * therefore be synchronized. Calling this method while the main.java.solver is in
    ...
    protected boolean doStep(SolutionStep step) {
        ...
        if (handled) {
            // main.java.solver has been used
            lastUsed = System.currentTimeMillis();
        }
        return handled;
    }
     ...
     * {@link #checkKrakenTypeOne(SudokuSet, int, int)} or
     * {@link #checkKrakenTypeTwo(SudokuSet, SudokuSet, int, int)}
...
//            }
     ...
     * {@link #checkAllChainsForHouse(SudokuSet[])}.
     */
     ...
     * then used in {@link #checkEntryList(List)} to find chains that
    ...
    /**
     * Used by {@link #checkAllChainsForHouse(SudokuSet[])} to check
     ...
     * @param entryList
     */
    private void checkEntryList(List<solver.TableEntry> entryList) {
        // AND all onSets and all Offset and see,
        // if something remains.
        for (int i = 0; i < entryList.size(); i++) {
            solver.TableEntry entry = entryList.get(i);
...
//        System.out.println("replaceorcopystep: " + globalStep.toString(2));
     ...
     * @return
     */
    private String printEntryList(List<solver.TableEntry> entryList) {
     ...
     * @param off
     */
    private void checkTwoChains(solver.TableEntry on, solver.TableEntry off) {
     ...
     * @param entry
     */
    private void checkOneChain(solver.TableEntry entry) {
        if (entry.index == 0) {
...
//            System.out.println("  6");
     ...
     * @param entityTyp
     */
    private void checkHouseDel(solver.TableEntry entry, SudokuSet[] houseSets, int entityTyp) {
        // check all candidates
        for (int i = 1; i < entry.offSets.length; i++) {
            // in all houses
     ...
     * @param entityTyp
     */
    private void checkHouseSet(solver.TableEntry entry, SudokuSet[] houseSets, int entityTyp) {
        for (int i = 1; i < entry.onSets.length; i++) {
            for (int j = 0; j < houseSets.length; j++) {
     ...
     * @param tables
     */
    private void checkNiceLoops(solver.TableEntry[] tables) {
     ...
     */
    private void checkAics(solver.TableEntry[] tables) {
        for (int i = 0; i < tables.length; i++) {
            int startIndex = tables[i].getCellIndex(0);
...
//                            && sudoku.isCandidate(startIndex, tables[i].getCandidate(j))) {
     ...
     *      in the table)
     */
    private void checkNiceLoop(solver.TableEntry entry, int entryIndex) {
...
//            steps.add((SolutionStep) globalStep.clone());
     ...
     */
    private void checkAic(solver.TableEntry entry, int entryIndex) {
...
//            }
    ...
    private void fillTablesWithGroupNodes() {
        ...
        for (int i = 0; i < groupNodes.size(); i++) {
            ...
            // one table for ON
            solver.TableEntry onEntry = getNextExtendedTableEntry(extendedTableIndex);
            ...
            // and one for OFF
            solver.TableEntry offEntry = getNextExtendedTableEntry(extendedTableIndex);
            ...
            if (!tmpSet.isEmpty()) {
                ...
                for (int j = 0; j < tmpSet.size(); j++) {
                    ...
                    solver.TableEntry tmp = onTable[index * 10 + gn.cand];
                ...
                if (!tmpSet1.isEmpty() && tmpSet1.size() == 1) {
                    ...
                    solver.TableEntry tmp = offTable[tmpSet1.get(0) * 10 + gn.cand];
        ...
        for (int alsIndex = 0; alsIndex < alses.size(); alsIndex++) {
            ...
            for (int entryCand = 1; entryCand <= 9; entryCand++) {
                ...
                for (int i = 0; i < tmpSet.size(); i++) {
                    ...
                    solver.TableEntry tmp = onTable[actIndex * 10 + entryCand];
                ...
                for (int i = 0; i < groupNodes.size(); i++) {
                    ...
                    solver.TableEntry gTmp = extendedTable.get(extendedTableMap.get(entry));
                    if (gTmp != null) {
                ...
                for (int actAlsIndex = 0; actAlsIndex < alses.size(); actAlsIndex++) {
                    ...
                    for (int actCand = 1; actCand <= 9; actCand++) {
                        ...
                        if (getAlsTableEntry(tmpAlsIndex, actAlsIndex, actCand) == null) {
                            solver.TableEntry tmpAlsEntry = getNextExtendedTableEntry(extendedTableIndex);
     ...
     * @return
     */
    private solver.TableEntry getAlsTableEntry(int entryCellIndex, int alsIndex, int cand) {
    ...
    /**
     * Returns the next free {@link solver.TableEntry } from {@link #extendedTable }
     ...
     * @return
     */
    private solver.TableEntry getNextExtendedTableEntry(int tableIndex) {
        solver.TableEntry entry;
        if (tableIndex >= extendedTable.size()) {
            entry = new solver.TableEntry();
     ...
     *
     * @param entry The {@link solver.TableEntry}
     ...
     * <code>false</code>
     */
    private void getTableEntry(solver.TableEntry entry, int cellIndex, int cand, boolean set) {
        if (DEBUG) {
     ...
     * @param entry {@link solver.TableEntry} for the operation
     ...
     */
    private void setCell(int cellIndex, int cand, solver.TableEntry entry,
...
//                }
     ...
     * {@link #setCell(int, int, com.company.solver.TableEntry, boolean, boolean)}.
     *
     * @param cellIndex
     * @param cand
     * @param houseSet
...
//    }
    ...
    /**
     * Expands the tables: every {@link solver.TableEntry } contains all direct
     ...
     */
    private void expandTable(solver.TableEntry dest, int index, int cand, boolean isOn,
...
//                        if (dest.isExpanded(orgIndex) && dest.getDistance(orgIndex) > (srcBaseDistance + srcDistance)) {
                    if (dest.isExpanded(orgIndex)
                        ...
                        dest.retIndices[orgIndex] = solver.TableEntry.makeSRetIndex(srcTableIndex, 0, 0, 0, 0);
...
//            printTable("r7c1=8", onTable[548], alses);
     ...
     * </ul> 
     * 
     * The search is done via the {@link solver.TableEntry#offSets} of
     * <code>src</code>.<br><br>
     *
     * If a new outcome is found, it is added to
     * <code>src</code> and all predecessors are recorded in the entries
     * {@link solver.TableEntry#retIndices return index}.
     *
     * @param src The {@link solver.TableEntry} which is currently handled
     ...
     */
    private void createNet(solver.TableEntry src, int srcIndex, int srcCand, boolean isOn) {
        if (DEBUG) {
...
//                printTable("r7c1=8", onTable[548], alses);
     ...
     */
    private void makeNetEntry(solver.TableEntry src, int srcIndex, int srcCand, boolean isOn,
...
//                System.out.println("   overwrite old entry");
                ...
                if (src.isExpanded(atIndex)) {
                    ...
                    newRetIndices = solver.TableEntry.makeSRetIndex(oldRetIndex,
                            retIndices[0][0], retIndices[0][1], retIndices[0][2],
                            retIndices[0][3]);
                } else {
                    newRetIndices = solver.TableEntry.makeSRetIndex(retIndices[0][0],
...
//                    printTable("??", onTable[548], alses);
     ...
     * are set.
     * 
     * @param src The {@link solver.TableEntry} of the source.
     ...
     */
    private void checkGroupNodeNetEntry(solver.TableEntry src, int srcIndex, int srcCand,
     ...
     * @param set
     */
    private void addChain(solver.TableEntry entry, int cellIndex, int cand, boolean set) {
     ...
     * @param isNiceLoop
     */
    private void addChain(solver.TableEntry entry, int cellIndex, int cand, boolean set, boolean isNiceLoop) {
     ...
     */
    private void addChain(solver.TableEntry entry, int cellIndex, int cand, boolean set, boolean isNiceLoop, boolean isAic) {
...
//        }
     ...
     * {@link #buildChain(solver.TableEntry, int, int[], boolean, SudokuSet)}.
     ...
     */
    private void buildChain(solver.TableEntry entry, int cellIndex, int cand, boolean set) {
        if (DEBUG) {
     ...
     *          {@link solver.TableEntry} where the first element (implication) of
     *          the min came from.
     ...
     * @return
     */
    private int buildChain(solver.TableEntry entry, int entryIndex, int[] actChain, boolean isMin,
                           solver.TableEntry minEntry, int minIndex, SudokuSet chainSet) {
        if (DEBUG) {
            doDebugCounter++;
        ...
        solver.TableEntry orgEntry = entry;
...
//                int retEntry = entry.entries[firstEntryIndex];
     ...
     * @param alses  
     */
    public static void printTables(String title, solver.TableEntry[] onTable, solver.TableEntry[] offTable, List<Als> alses) {
        if (!DEBUG) {
            return;
        }
        System.out.println("==========================================");
    ...
    /**
     * Show the contents of one {@link solver.TableEntry} (for debugging).
     *
     * @param title
     * @param entry
     * @param alses  
     */
    public static void printTable(String title, solver.TableEntry entry, List<Als> alses) {
        if (!DEBUG) {
            return;
        }
        System.out.println(title + ": ");
...
//        }
    ...
    /**
     * Show one {@link solver.TableEntry} (for debugging).
     *
     * @param entry
     * @param alses 
     * @return
